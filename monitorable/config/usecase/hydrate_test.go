...
`
	...
	store := cache.NewGoCacheStore(time.Second, time.Second)
	usecase := initConfigUsecase(nil, store)
	usecase.RegisterMetaTile(jenkins.JenkinsMultiBranchTileType, DefaultVariant, &_jenkinsModels.MultiBranchParams{}, mockBuilder)
...
func TestUsecase_Hydrate_WithMetaEmpty(t *testing.T) {
	input := `
{
  "columns": 4,
  "tiles": [
...
`
	...
	usecase.RegisterTile(jenkins.JenkinsBuildTileType, "variant1", &_jenkinsModels.BuildParams{}, "/jenkins/variant1", 1000)
	usecase.RegisterMetaTile(jenkins.JenkinsMultiBranchTileType, DefaultVariant, &_jenkinsModels.MultiBranchParams{}, mockBuilder)
	usecase.RegisterMetaTile(jenkins.JenkinsMultiBranchTileType, "variant1", &_jenkinsModels.MultiBranchParams{}, mockBuilder2)
...
`
	...
	cacheKey := fmt.Sprintf("%s:%s_%s_%s", MetaTileStoreKeyPrefix, "JENKINS-MULTIBRANCH", "default", `{"job":"test"}`)
	_ = usecase.metaTileStore.Add(cacheKey, cachedResult, 0)
